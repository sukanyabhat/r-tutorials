...
#biopics_based_on_437<-biopics %>% filter(box_office != '-')
biopics<-biopics %>% mutate(box_office_clean=ifelse(box_office == '-', NA, box_office)) %>% mutate(box_office_clean=gsub("[^0-9\\.]", "",box_office_clean))
#biopics %>% select(box_office_clean,ends_with('M'))
ggplot(data=biopics, aes(as.numeric(biopics$box_office_clean))) + geom_density() + facet_wrap(~subject_race,scales = "free_y") 
...
prop.table(table(biopics$type_of_subject,biopics$subject_race))
